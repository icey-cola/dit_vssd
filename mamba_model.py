...
from vssd_structure import Mamba2
...
class PatchEmbed(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = rearrange(x, 'b h w c ->  b(h w) c',  b=B, h=num_patches, w=num_patches)
        return x
...
class DiT_VSSD_Block(nn.Module):
    """
    ...
    def __call__(self, x, c):
        ...
        # x_modulated = rearrange(x_modulated, 'b h w c -> b (h w) c', h=H, w=W)

        attn_x = Mamba2(d_model=self.hidden_size,
                    expand=int(self.mlp_ratio),
                    headdim=int(self.hidden_size * self.mlp_ratio) // self.num_heads)(
                        x_modulated, self.H, self.W, train=self.train
                        )
        # attn_x = rearrange(attn_x, 'b (h w) c -> b h w c', h=H, w=W)
...
class DiT(nn.Module):
    ...
    dit_schedule: Tuple[str, ...] = ('mamba', 'mamba', 'mamba', 'mamba', 
    ...
    def __call__(self, x, t, dt, y, train=False, return_activations=False):
        ...
        # if c is (B, hidden_size),  will do nothing, but if it is (1, B, hidden_size), we shou remove the first dimension 
        # what if B = 1?, then squeeze will remove the first dimension, and the shape will be (hidden_size,), which is not what we want.
        c = (te + ye + dte) 
        if len(c.shape) != 2:
            c = c.squeeze(0)
        ...
        for i, block_type in zip(range(self.depth), self.dit_schedule):
            if block_type == 'mamba':
                x = DiT_VSSD_Block(self.hidden_size, self.num_heads, tc, self.mlp_ratio, self.dropout, train)(x, c)
